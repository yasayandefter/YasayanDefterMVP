...
function mapAttempt(row) { if (!row) return null; const iso = value => value instanceof Date ? value.toISOString() : value || null; return { id: row.id, studentId: row.student_id, ownerUserId: row.owner_user_id || null, topic: row.topic, difficulty: row.difficulty, questionType: row.question_type, status: row.status, score: Number(row.score || 0), xpAwarded: Number(row.xp_awarded || 0), startedAt: iso(row.started_at), completedAt: iso(row.completed_at), createdAt: iso(row.created_at), updatedAt: iso(row.updated_at) }; }
...
async function createAttempt({ id = crypto.randomUUID(), studentId, topic, difficulty = "medium", questionType = "multiple-choice" }, client = db) { const scope = owner(studentId); try { const result = await client.query("INSERT INTO quiz_attempts (id, student_id, owner_user_id, topic, difficulty, question_type, status) VALUES ($1,$2,$3,$4,$5,$6,'ACTIVE') RETURNING *", [id, scope.studentId, scope.userId, topic, difficulty, questionType]); return mapAttempt(result.rows[0]); } catch (error) { throw mapDatabaseError(error, "QUIZ_ATTEMPT_CREATE_FAILED"); } }
...
async function findAttemptForStudent(id, studentId, client = db) { const scope = owner(studentId); const result = await client.query(`SELECT * FROM quiz_attempts WHERE id = $1 AND ${scope.column} = $2`, [id, scope.id]); return mapAttempt(result.rows[0]); }
...
async function findQuestion(attemptId, questionId, client = db) { const result = await client.query("SELECT * FROM quiz_attempt_questions WHERE attempt_id = $1 AND id = $2", [attemptId, questionId]); return result.rows[0] || null; }
async function listQuestions(attemptId, client = db) { const result = await client.query("SELECT id, attempt_id, ordinal, prompt, options, concept FROM quiz_attempt_questions WHERE attempt_id = $1 ORDER BY ordinal", [attemptId]); return result.rows; }
async function completeAttempt({ attemptId, studentId, score, xpAmount = 0 }, client = db) {
  ...
  const operation = async transactionClient => {
    const locked = await transactionClient.query(`SELECT * FROM quiz_attempts WHERE id = $1 AND ${scope.column} = $2 FOR UPDATE`, [attemptId, scope.id]);
    ...
    const updated = await transactionClient.query(`UPDATE quiz_attempts SET status = 'COMPLETED', score = $3, xp_awarded = $4, completed_at = NOW(), updated_at = NOW() WHERE id = $1 AND ${scope.column} = $2 AND status <> 'COMPLETED' RETURNING *`, [attemptId, scope.id, Number(score) || 0, Number(xpAmount) || 0]);
    const xp = await transactionClient.query("INSERT INTO xp_events (id, student_id, owner_user_id, attempt_id, amount, reason) VALUES ($1,$2,$3,$4,$5,'quiz') ON CONFLICT (attempt_id) DO NOTHING RETURNING id, amount", [crypto.randomUUID(), scope.studentId, scope.userId, attemptId, Number(xpAmount) || 0]);
...
async function findRecentAttempts(studentId, options = {}, client = db) { const scope = owner(studentId); const limit = page(options.limit); const offset = Math.max(0, Number.parseInt(options.offset, 10) || 0); const result = await client.query(`SELECT * FROM quiz_attempts WHERE ${scope.column} = $1 ORDER BY created_at DESC, id LIMIT $2 OFFSET $3`, [scope.id, limit, offset]); return result.rows.map(mapAttempt); }
...
module.exports = { name: "quiz", mapAttempt, createAttempt, findAttemptById, findAttemptForStudent, insertQuestions, recordAnswer, listAnswers, findQuestion, listQuestions, completeAttempt, findRecentAttempts, findAttempt, saveAttempt, getProgressSource, findXpByAttempt };
