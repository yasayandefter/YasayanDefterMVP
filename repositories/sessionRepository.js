...
async function findValidSession(token, client = db) { if (!token) return null; const result = await client.query("SELECT s.id AS session_id, s.user_id, s.expires_at, u.role, u.email, u.username, u.display_name, u.status, st.id AS student_id FROM sessions s JOIN users u ON u.id = s.user_id LEFT JOIN students st ON st.user_id = u.id WHERE s.session_hash = $1 AND s.revoked_at IS NULL AND s.expires_at > NOW() AND UPPER(u.status) = 'ACTIVE'", [hashToken(token)]); return result.rows[0] || null; }
