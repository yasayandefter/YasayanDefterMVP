...
const { assertIsolatedTestDatabase } = require("./helpers/testDatabaseGuard");

const testDatabaseIdentity = assertIsolatedTestDatabase(process.env);
...
(async()=>{
  const identity=await pool.query("SELECT current_database() database,current_user username");assert.equal(identity.rows[0].database,testDatabaseIdentity.database);assert.equal(identity.rows[0].username,testDatabaseIdentity.username);await seed();
  const p=await port(),base=`http://localhost:${p}`;let server=start(baseEnv(p));let health=await wait(base);let status=await health.json();assert.equal(status.status,"ok");assert.equal(status.storageMode,"postgres");assert.equal(status.storageHealth,"ok");assert.equal(JSON.stringify(status).includes("DATABASE_URL"),false);
  ...
  const outagePort=await port();const unreachable=["postgresql:","//invalid:invalid@127.0.0.1:1/unreachable"].join("");const outage=start({...baseEnv(outagePort),DATABASE_URL:unreachable,PG_CONNECTION_TIMEOUT_MS:"1000"});const degraded=await wait(`http://localhost:${outagePort}`,200);const degradedBody=await degraded.json();assert.equal(degradedBody.storageHealth,"unavailable");assert.equal(degradedBody.status,"degraded");outage.output();assertStopped(await stop(outage,"SIGTERM"),"SIGTERM");
