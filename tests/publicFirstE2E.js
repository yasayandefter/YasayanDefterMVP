...
(async () => {
  ...
  const emptyFixture = { query: currentTechnologyQuery, title: currentTechnologyQuery, mode: "current", researchMode: "current", currentState: "CURRENT_EMPTY", currentSourceCount: 0, freshness: { checkedAt: "2026-08-12T12:00:00.000Z", sourceCount: 0 }, brain: { category: "Teknoloji", facts: [], flashcards: [] }, ai: { lesson: null }, articles: [], sources: [], reliability: { score: 0, level: "low", sourceCount: 0, independentDomainCount: 0, highQualitySourceCount: 0 }, structuredContent: { summary: "Bu konu için güncel ve doğrulanabilir bir kaynak şu anda bulunamadı.", sections: [], keyFacts: [], keyConcepts: [], interestingFacts: [], followUpQuestions: [{ text: "Teknoloji haberlerini hangi kaynaklardan takip edebilirim?", query: "Teknoloji haberlerini hangi kaynaklardan takip edebilirim?" }, { text: "Yapay zekâ alanındaki son gelişmeleri araştır.", query: "Yapay zekâ alanındaki son gelişmeleri araştır." }, { text: "Bugünkü uzay teknolojisi gelişmelerini araştır.", query: "Bugünkü uzay teknolojisi gelişmelerini araştır." }], limitations: ["Güncel kaynak bulunamadı."] } };
  await page.evaluate(data => renderResearch(data), emptyFixture); const emptyText = await page.locator("#professionalResult").innerText(); assert.equal(emptyText.includes("[object Object]"), false); for (const question of emptyFixture.structuredContent.followUpQuestions.map(item => item.text)) assert.equal(emptyText.includes(question), true, question); assert.equal(await page.locator("#professionalResult[data-current-empty='true']").count(), 1); assert.equal(await page.locator("#results > .section:not(#professionalResult):visible").count(), 0); assert.equal(await page.locator(".current-empty-retry:visible").count(), 1);
  const retryResponse = page.waitForResponse(r => r.url().includes("/api/research") && r.status() === 200); await page.locator(".current-empty-retry").click(); const retryPayload = await (await retryResponse).json(); assert.equal(retryPayload.query, currentTechnologyQuery); assert.equal(await page.locator("#questionInput").inputValue(), currentTechnologyQuery); await page.waitForTimeout(500);
  ...
  assert.equal(await page.locator("#professionalResult[data-current-empty='true']").count(), 0); assert.ok(await page.locator("#professionalResult .professional-fact-card:visible").count() > 0); assert.ok(await page.locator("#professionalResult .professional-source-card:visible").count() > 0); await page.evaluate(() => window.YDResearchWorkspace.activate("overview", false)); assert.ok(await page.locator("#yd-research-panel-overview > .section:visible").count() > 0);
  await page.locator("#questionInput").fill("Toryum nedir?"); const standardRegressionResponse = page.waitForResponse(r => r.url().includes("/api/research") && r.status() === 200); await page.locator("#searchButton").click(); const standardRegressionPayload = await (await standardRegressionResponse).json(); await page.locator("#results.visible").waitFor(); assert.equal(standardRegressionPayload.mode, "standard"); await page.evaluate(() => window.YDResearchWorkspace.activate("overview", false)); assert.equal(await page.locator("#teacherSimple").isVisible(), true); await page.evaluate(() => window.YDResearchWorkspace.activate("memory", false)); assert.equal(await page.locator("#flashcardsContainer").isVisible(), true); await page.evaluate(() => window.YDResearchWorkspace.activate("quiz", false)); assert.equal(await page.locator("#quizQuestion").isVisible(), true);
