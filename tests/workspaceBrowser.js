...
(async () => {
  ...
  const preferences = page.locator('.yd-profile-fold').filter({ has: page.locator('#workspaceSettings') });
  if (await preferences.count() && !(await preferences.evaluate(e => e.open))) await preferences.locator('summary').click();
