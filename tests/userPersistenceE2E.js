...
(async () => {
  ...
  await page.goto(base); await page.locator("[data-open-register]").click(); await page.getByLabel("Kullanıcı adı", { exact: true }).fill(username); await page.getByLabel("Parola", { exact: true }).fill(password); await page.getByLabel("Parola tekrar").fill(password); await page.getByRole("button", { name: "Hesap oluştur" }).click(); await page.locator("[data-auth-user]").waitFor();
  ...
  await page.reload(); await page.locator("[data-auth-user]").waitFor(); await page.waitForFunction(() => document.querySelector("#notebookList")?.textContent.toLocaleLowerCase("tr-TR").includes("venüs"));
  await logout(page); await login(page); await page.waitForFunction(() => document.querySelector("#notebookList")?.textContent.toLocaleLowerCase("tr-TR").includes("venüs")); assert.equal(await page.evaluate(() => localStorage.getItem("yasayanDefterNotebook")), null);
  assert.deepEqual(consoleErrors, []); assert.deepEqual(pageErrors, []); await context.close();
  console.log("PASS  real Edge authenticated USER Defterim save, refresh, logout/login restore, server ownership, and no local notebook shadow");
