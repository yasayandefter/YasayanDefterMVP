...
(async () => {
  ...
  await page.locator("#questionInput").fill("Venüs gezegeni"); const research = page.waitForResponse(response => new URL(response.url()).pathname === "/api/research" && response.status() === 200); await page.locator("#searchButton").click(); await research; await page.locator("#results.visible").waitFor();
  ...
  await page.waitForFunction(() => window.YasayanDefterAuth?.authenticated === true && window.YasayanDefterAccess?.isDemoMode() === false);
  const save = page.waitForResponse(response => response.url().includes("/api/memory/save")); await page.locator("#saveTopicButton").evaluate(button => button.click()); const saveResponse = await save; assert.equal(saveResponse.ok(), true); assert.match(await page.locator("#saveTopicButton").textContent(),/Deftere kaydedildi|Defterde kayıtlı/);
  ...
  await navigateTo(page, "notebook");
