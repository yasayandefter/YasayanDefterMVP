...
(async () => {
  ...
  for (const viewport of [{ width: 360, height: 800 }, { width: 390, height: 844 }, { width: 768, height: 900 }, { width: 1024, height: 768 }, { width: 1366, height: 768 }, { width: 1536, height: 768 }]) {
  ...
  if (process.env.TEST_DATABASE_URL) {
    ...
    assert.equal(await authPage.locator(".yd-overview-learning").isVisible(), true);
    await authPage.locator("#topicImageBox img").waitFor({ state: "visible", timeout: 15000 }).catch(() => {});
    const visibleResult = await authPage.evaluate(() => Object.fromEntries(["topicTitle", "heroSummary"].map(id => { const node = document.getElementById(id), rect = node?.getBoundingClientRect(); return [id, { text: node?.textContent?.trim() || "", width: rect?.width || 0, top: rect?.top ?? innerHeight, bottom: rect?.bottom ?? innerHeight }]; })));
    assert.equal(visibleResult.topicTitle.text.toLocaleLowerCase("tr-TR"), "mustafa kemal atatürk");
    ...
    const commercialLayout = await authPage.evaluate(() => {
      ...
      return { command: box(".hero-search"), hero: box("#topicTitle"), summary: box("#heroSummary"), image: box("#topicImageBox img"), metadata: box(".hero-info"), toolbar: box(".yd-research-tabs"), discovery: box(".yd-overview-discovery"), discoveryImage: box(".yd-discovery-images img"), learning: box(".yd-overview-learning"), continuation: box("#followContainer"), overflow: document.documentElement.scrollWidth - document.documentElement.clientWidth, bodyRatio: document.documentElement.scrollHeight / innerHeight, overviewScroll: (overview?.scrollHeight || 0) - (overview?.clientHeight || 0), overviewOverflow: getComputedStyle(overview).overflowY };
    });
    assert.ok(commercialLayout.continuation.bottom <= 960, JSON.stringify(commercialLayout));
    assert.ok(commercialLayout.command.height <= 70, JSON.stringify(commercialLayout));
    for (const key of ["hero", "summary", "metadata", "toolbar", "learning"]) assert.equal(commercialLayout[key].visible, true, `${key}: ${JSON.stringify(commercialLayout)}`);
    if (commercialLayout.discoveryImage.width > 0) assert.equal(commercialLayout.discovery.visible, true);
    if (commercialLayout.image.width > 0) assert.equal(commercialLayout.image.visible, true, JSON.stringify(commercialLayout));
    assert.ok(commercialLayout.overflow <= 0 && commercialLayout.bodyRatio <= 1.25, JSON.stringify(commercialLayout));
    ...
    await authPage.waitForFunction(() => document.getElementById("topicTitle")?.textContent?.trim().toLocaleLowerCase("tr-TR") === "mars", {}, { timeout: 120000 });
    assert.equal(await authPage.locator('[data-research-tab="overview"]').getAttribute("aria-selected"), "true");
    ...
    for (const viewport of [{ width: 360, height: 800 }, { width: 390, height: 844 }, { width: 768, height: 900 }, { width: 1024, height: 768 }, { width: 1366, height: 768 }, { width: 1536, height: 768 }]) {
      ...
      const layout = await authPage.evaluate(() => ({ overflow: document.documentElement.scrollWidth - document.documentElement.clientWidth, ratio: document.documentElement.scrollHeight / innerHeight, topic: document.querySelector("#topicTitle")?.getBoundingClientRect(), discovery: document.querySelector(".yd-overview-discovery")?.getBoundingClientRect(), learning: document.querySelector(".yd-overview-learning")?.getBoundingClientRect(), follow: document.querySelector("#followContainer")?.getBoundingClientRect() }));
      assert.ok(layout.overflow <= 0, `${viewport.width}: ${JSON.stringify(layout)}`);
      assert.ok(layout.topic?.width > 0 && layout.learning?.width > 0 && layout.follow?.width > 0, `${viewport.width}: ${JSON.stringify(layout)}`);
      if (await authPage.locator('.yd-discovery-images img').count()) assert.ok(layout.discovery?.width > 0);
      if (viewport.width === 1024 || viewport.width === 1366 || viewport.width === 1536) assert.ok(layout.ratio <= 1.25, `${viewport.width}: ${JSON.stringify(layout)}`);
    }
    await authPage.setViewportSize({ width: 1366, height: 768 });
    authRatio = await authPage.evaluate(() => document.documentElement.scrollHeight / innerHeight); assert.ok(authRatio <= 1.25, `AUTH_BODY_RATIO_${authRatio}`); assert.equal(await authPage.locator("[data-research-save]").isVisible(), true); assert.deepEqual(authErrors, []); await authContext.close();
  }
  console.log(`PASS  real Edge 15.8 V4 dedicated visuals/quiz/sources and 360/390/768/1024/1366/1536 bounded layouts; public ratio=${ratio.toFixed(3)}${authRatio === null ? "" : `; auth body ratio=${authRatio.toFixed(3)}`}`);
