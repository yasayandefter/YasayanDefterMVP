...
(async () => {
  ...
  await page.goto(base); await page.getByRole("button", { name: /Hesap oluştur/ }).click(); await page.getByLabel("Kullanıcı adı", { exact: true }).fill(username); await page.getByLabel("E-posta \(opsiyonel\)").fill(email); await page.getByLabel("Parola", { exact: true }).fill(oldPassword); await page.getByLabel("Parola tekrar").fill(oldPassword); await page.getByRole("button", { name: "Hesap oluştur" }).click(); await page.locator(".workspace-dialog[open]").waitFor(); await page.getByRole("button", { name: "Şimdilik geç" }).click(); await page.locator(".workspace-dialog").waitFor({ state: "hidden" }); await page.locator("#editProfileButton").waitFor({ state: "visible" });
