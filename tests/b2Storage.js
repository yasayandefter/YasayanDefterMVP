...
(async () => {
  ...
  const upload = await storage.createUploadAuthorization({ key, mimeType: "application/pdf" });
  assert.deepEqual(upload.headers, { "Content-Type": "application/pdf" }); assert.equal(upload.expiresInSeconds, 600); assert.equal(signed[0].options.expiresIn, 600); assert.deepEqual(signed[0].command.input, { Bucket: env.B2_BUCKET_NAME, Key: key, ContentType: "application/pdf" });
  ...
  console.log("PASS  B2 config, selection, endpoint/region/bucket, virtual-host addressing, signed PUT/GET TTL and MIME, HEAD, delete, key security and credential isolation");
