...
(async () => {
  ...
  const upload = await storage.createUploadAuthorization({ key: "users/hash/media/id/file.pdf", mimeType: "application/pdf" });
  assert.equal(upload.method, "PUT"); assert.deepEqual(upload.headers, { "Content-Type": "application/pdf" }); assert.equal(signed[0].options.expiresIn, 600); assert.equal(signed[0].command.input.Bucket, "private-bucket"); assert.equal(signed[0].command.input.ContentType, "application/pdf");
  ...
  console.log("PASS  R2 adapter one-object PUT/GET signing, Content-Type binding, HEAD verification, delete and credential isolation");
