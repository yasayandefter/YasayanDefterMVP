...
const records = new Map(); let reservationFailure = null, completionFailure = null;
const repository = {
  async reservePending(value, limits) { if (reservationFailure) return { asset: null, reason: reservationFailure }; assert.equal(limits.maxOutstandingUploadsPerUser, 3); const asset = { ...value, status: "PENDING", createdAt: "2026-08-29T00:00:00.000Z", updatedAt: "2026-08-29T00:00:00.000Z" }; records.set(value.id, asset); return { asset }; },
  async findOwned(id, userId) { const asset = records.get(id); return { exists: Boolean(asset), asset: asset?.userId === userId ? asset : null }; },
  async finalizeReady(id, userId, limits, extra) { const asset = records.get(id); if (completionFailure) return { asset, reason: completionFailure }; Object.assign(asset, { status: "READY", objectEtag: extra.etag }); return { asset, reason: null }; },
...
function pending(id, mimeType, mediaType, sizeBytes) { return { id, userId: user.userId, storageProvider: "r2", storageKey: `users/hash/media/${id}/file`, originalFilename: "file", safeFilename: "file", mimeType, mediaType, sizeBytes, status: "PENDING", createdAt: new Date().toISOString(), updatedAt: new Date().toISOString() }; }
...
(async () => {
  ...
  assert.equal(getMediaConfig({ ...secretConfig, MEDIA_MAX_OUTSTANDING_UPLOADS_PER_USER: "0" }).errorCode, "MEDIA_STORAGE_CONFIG_INVALID");
  ...
  assert.equal(storage.calls[0].sizeBytes, 2048); assert.deepEqual(requested.upload.signedHeaders, ["content-length", "content-type"]);
  ...
  assert.equal(records.get(ids[0]).status, "DELETING"); assert.equal(storage.objects.has(records.get(ids[0]).storageKey), false);

  const shortId = "55555555-5555-4555-8555-555555555555", mimeId = "66666666-6666-4666-8666-666666666666", policyId = "77777777-7777-4777-8777-777777777777";
  records.set(shortId, pending(shortId, "image/png", "IMAGE", 100)); storage.objects.set(records.get(shortId).storageKey, { sizeBytes: 99, mimeType: "image/png" }); await assert.rejects(service.completeUpload(user, shortId), error => error.code === "MEDIA_OBJECT_MISMATCH"); assert.equal(records.get(shortId).status, "DELETING");
  records.set(policyId, pending(policyId, "image/png", "IMAGE", 10 * 1024 * 1024 + 1)); storage.objects.set(records.get(policyId).storageKey, { sizeBytes: 10 * 1024 * 1024 + 1, mimeType: "image/png" }); await assert.rejects(service.completeUpload(user, policyId), error => error.code === "MEDIA_OBJECT_MISMATCH");
  records.set(mimeId, pending(mimeId, "image/png", "IMAGE", 100)); const mismatchStorage = createMockObjectStorage({ objects: { [records.get(mimeId).storageKey]: { sizeBytes: 100, mimeType: "image/jpeg" } }, failDelete: true }); const mismatchService = createMediaService({ repository, objectStorage: mismatchStorage, config }); await assert.rejects(mismatchService.completeUpload(user, mimeId), error => error.code === "MEDIA_OBJECT_MISMATCH"); assert.equal(records.get(mimeId).status, "DELETING"); assert.equal(mismatchStorage.objects.has(records.get(mimeId).storageKey), true);
  ...
  reservationFailure = "QUOTA"; await assert.rejects(service.requestUpload(user, { filename: "x.png", mimeType: "image/png", sizeBytes: 100 }), error => error.code === "MEDIA_QUOTA_EXCEEDED");
  reservationFailure = "OUTSTANDING"; await assert.rejects(service.requestUpload(user, { filename: "x.png", mimeType: "image/png", sizeBytes: 100 }), error => error.code === "MEDIA_OUTSTANDING_LIMIT");
  reservationFailure = "INIT_RATE"; await assert.rejects(service.requestUpload(user, { filename: "x.png", mimeType: "image/png", sizeBytes: 100 }), error => error.code === "MEDIA_UPLOAD_INIT_RATE_LIMIT");
  reservationFailure = "COLLECTION_CAPACITY"; await assert.rejects(service.requestUpload(user, { filename: "x.png", mimeType: "image/png", sizeBytes: 100, collectionId: ids[3] }), error => error.code === "COLLECTION_CAPACITY_EXCEEDED"); reservationFailure = null;

  const signId = "88888888-8888-4888-8888-888888888888", signingService = createMediaService({ repository, objectStorage: { available: true, async createUploadAuthorization() { throw new Error("SIGNING_FAILED"); } }, config, randomUUID: () => signId }); await assert.rejects(signingService.requestUpload(user, { filename: "sign.png", mimeType: "image/png", sizeBytes: 100 }), error => error.code === "MEDIA_STORAGE_UNAVAILABLE"); assert.equal(records.has(signId), false);

  records.get(second.asset.id).status = "PENDING"; completionFailure = "QUOTA"; storage.objects.set(records.get(second.asset.id).storageKey, { sizeBytes: 4096, mimeType: "image/webp" }); await assert.rejects(service.completeUpload(user, second.asset.id), error => error.code === "MEDIA_QUOTA_EXCEEDED"); assert.equal(records.get(second.asset.id).status, "DELETING"); completionFailure = null;

  const failedStorage = createMockObjectStorage({ failDelete: true });
  records.get(second.asset.id).status = "READY"; const failedDeleteService = createMediaService({ repository, objectStorage: failedStorage, config });
