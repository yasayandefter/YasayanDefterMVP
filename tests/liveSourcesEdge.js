...
(async () => {
  ...
  for (const query of queries) {
    ...
    const domText = await page.locator("#results").innerText(); const payloadText = JSON.stringify(payload); for (const banned of ["<p>", "<table", "&nbsp;", "href=", "View CSAF", "Summary Successful", "Description Successful", "Overview Successful", "Kaynak ile ilgili doğrulanmış gelişme neydi?", "Bilgiye daha hızlı ulaşmayı sağlar", "günlük hayatta kullandığın bir araç gibi düşün", "Bu konu tamamen hayal ürünüdür"]) { assert.equal(domText.includes(banned), false, `${query}: ${banned}`); assert.equal(payloadText.includes(banned), false, `${query} payload: ${banned}`); }
