...
async function cleanup() {
  ...
  await pool.query("DELETE FROM research_activity_events WHERE student_id=ANY($1::uuid[])", [students]);
...
(async () => {
  ...
  const beforeDemo = await pool.query("SELECT (SELECT count(*) FROM users) users,(SELECT count(*) FROM students) students,(SELECT count(*) FROM memory_records) memories,(SELECT count(*) FROM research_activity_events) research_events,(SELECT count(*) FROM quiz_attempts) quizzes,(SELECT count(*) FROM xp_events) xp,(SELECT count(*) FROM classrooms) classrooms").then(r=>r.rows[0]);
  ...
  await demo.goto(base); await demo.locator("[data-open-login]").waitFor(); assert.equal(await demo.locator(".auth-shell").isVisible(),false); assert.equal(await demo.locator("#commercialStreak").textContent(),"—"); assert.match(await demo.locator("#commercialGoalLabel").textContent(),/Giriş yapınca takip edilir/); assert.deepEqual(errors.console,[]); assert.deepEqual(errors.page,[]); assert.deepEqual(errors.network,[]); errors.requests.length=0;
  assert.equal(await demo.evaluate(()=>window.YasayanDefterAccess.isDemoMode()),true);
  await demo.locator("#questionInput").fill("Satürn halkaları"); const demoResearch=demo.waitForResponse(r=>r.url().includes("/api/research")&&r.status()===200); await demo.locator("#searchButton").click(); await demoResearch; await demo.locator("#results.visible").waitFor(); assert.ok((await demo.locator("#summaryText").textContent()).trim().length>0); await demo.locator("#saveTopicButton").click(); assert.match(await demo.locator("#saveTopicButton").textContent(),/giriş yap/i); assert.equal(await demo.evaluate(()=>localStorage.getItem("yasayanDefterNotebook")),null); assert.equal(await demo.locator(".delete-save").count(),0);
  ...
  await demo.locator("[data-open-login]").click(); await demo.locator(".auth-shell").waitFor({state:"visible"}); assert.equal(await demo.locator(".auth-card").getAttribute("aria-modal"),"true"); await demo.keyboard.press("Escape"); await demo.locator(".auth-shell").waitFor({state:"hidden"}); assert.equal(await demo.evaluate(()=>document.activeElement?.hasAttribute("data-open-login")),true); await demoContext.close();
