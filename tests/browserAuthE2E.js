...
const fixture = { teacher: `browser-teacher-${suffix}@example.test`, student: `browser_student_${suffix}`, claimed: `browser_claimed_${suffix}`, password: "Phase6-browser-password!", claim: `P6-${crypto.randomBytes(8).toString("hex")}` };
const pool = new Pool({ connectionString: process.env.TEST_DATABASE_URL, max: 2 });
let child; let browser; let activePage; const diagnostics = { console: [], page: [], network: [] };
...
async function logout(page) { await page.getByRole("button", { name: "Çıkış yap" }).click(); await page.locator(".auth-shell").waitFor({ state: "visible" }); }
...
(async () => {
  ...
  await teacher.goto(base); await teacher.locator(".auth-shell").waitFor({state:"visible"});
  ...
  const claimContext = await browser.newContext({ viewport: { width: 390, height: 850 } }); const claimPage = await claimContext.newPage(); activePage=claimPage; observe(claimPage,errors,base); await claimPage.goto(base); await claimPage.getByRole("button",{name:/Öğrenci hesabını oluştur/}).click(); errors.console.length=0; errors.page.length=0; errors.network.length=0;
  ...
  await claimPage.getByLabel("Claim kodu").fill(fixture.claim); await claimPage.getByLabel("Kullanıcı adı",{exact:true}).fill(fixture.claimed); await claimPage.getByLabel("Parola",{exact:true}).fill(fixture.password); await claimPage.getByLabel("Parola tekrar").fill(fixture.password); await claimPage.getByRole("button",{name:"Hesabımı oluştur"}).click(); await claimPage.waitForLoadState("domcontentloaded"); await claimPage.locator("[data-auth-user]").waitFor();
  ...
  await claimPage.getByRole("button",{name:/Öğrenci hesabını oluştur/}).click(); await claimPage.getByLabel("Claim kodu").fill(fixture.claim); await claimPage.getByLabel("Kullanıcı adı",{exact:true}).fill(`reuse_${suffix}`); await claimPage.getByLabel("Parola",{exact:true}).fill(fixture.password); await claimPage.getByLabel("Parola tekrar").fill(fixture.password); await claimPage.getByRole("button",{name:"Hesabımı oluştur"}).click(); await claimPage.locator(".auth-message.is-error").waitFor(); assert.match(await claimPage.locator(".auth-message.is-error").textContent(),/kullan|claim|kod/i); errors.console.length=0; errors.page.length=0; errors.network.length=0; await claimContext.close(); activePage=null;

  assert.deepEqual(errors.console,[]); assert.deepEqual(errors.page,[]); assert.deepEqual(errors.network,[]);
  console.log("PASS  real Edge teacher, student, claim, session, cookie, responsive, accessibility, console, and cleanup browser E2E");
