...
(async () => {
  ...
  await demo.goto(base); await demo.locator(".auth-shell").waitFor({state:"visible"}); assert.equal(await demo.getByRole("button",{name:/Demoyu aç/}).isVisible(),true); assert.deepEqual(errors.console,[]); assert.deepEqual(errors.page,[]); assert.deepEqual(errors.network,[]); errors.requests.length=0;
  await demo.getByRole("button",{name:/Demoyu aç/}).click(); await demo.locator(".auth-demo-badge").waitFor(); assert.equal(await demo.evaluate(()=>window.YasayanDefterAccess.isDemoMode()),true);
  ...
  await demo.getByRole("button",{name:"Demodan çık"}).click(); await demo.locator(".auth-shell").waitFor({state:"visible"}); await demoContext.close();
  ...
  const claimContext = await browser.newContext({ viewport: { width: 390, height: 850 } }); const claimPage = await claimContext.newPage(); activePage=claimPage; observe(claimPage,errors,base); await claimPage.goto(base); await claimPage.getByRole("button",{name:/Davet kodunu kullan/}).click(); errors.console.length=0; errors.page.length=0; errors.network.length=0;
  ...
  await claimPage.getByRole("button",{name:/Davet kodunu kullan/}).click(); await claimPage.getByLabel("Davet / claim kodu").fill(fixture.claim); await claimPage.getByLabel("Kullanıcı adı",{exact:true}).fill(`reuse_${suffix}`); await claimPage.getByLabel("Parola",{exact:true}).fill(fixture.password); await claimPage.getByLabel("Parola tekrar").fill(fixture.password); await claimPage.getByRole("button",{name:"Hesabımı oluştur"}).click(); await claimPage.locator(".auth-message.is-error").waitFor(); assert.match(await claimPage.locator(".auth-message.is-error").textContent(),/kullan|claim|kod/i); errors.console.length=0; errors.page.length=0; errors.network.length=0; await claimContext.close(); activePage=null;
