...
function buildDryRun(root = process.cwd(), options = {}) {
  ...
  for (const row of map.quizAttempts.data) {
    const studentId = row?.studentId ? (studentIds.get(String(row.studentId)) || orphanMapping.get(String(row.studentId))) : mapLegacyDefault ? legacyStudentId : null; const attempt = normalizers.attempt(row, studentId); if (!studentId) conflicts.push(conflict("QUIZ_STUDENT_NOT_FOUND", "BLOCKING")); attempts.push(attempt);
...
function cli(argv = process.argv.slice(2)) { if (argv.includes("--apply")) throw new Error("APPLY_DISABLED_IN_PHASE_4"); const rootArg = argv.find(value => value.startsWith("--root=")); const root = rootArg ? path.resolve(rootArg.slice(7)) : process.cwd(); const report = buildDryRun(root, { mapLegacyDefault: argv.includes("--map-legacy-default"), reconcileOrphanStudents: argv.includes("--reconcile-orphan-students") }); const file = writeReport(report); process.stdout.write(JSON.stringify({ ok: true, mode: report.mode, ownershipMapping: report.ownership.enabled, orphanReconciliation: report.orphanReconciliation.enabled, migrationPlanValid: report.migrationPlanValid, blockingConflicts: report.blockingConflicts, warnings: report.warnings, reportFile: path.relative(process.cwd(), file).replace(/\\/g, "/") }) + "\n"); }
