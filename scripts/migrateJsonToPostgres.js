...
function buildDryRun(root = process.cwd(), options = {}) {
  ...
  if (students.some(row => !row.classroomId)) conflicts.push(conflict("STUDENT_CLASSROOM_NOT_FOUND", "BLOCKING", students.filter(row => !row.classroomId).length));
  if (students.length) conflicts.push(conflict("LEGACY_STUDENT_ACCOUNT_REQUIRED", "WARNING", students.length));
  const memoryRows = []; let unassignedMemory = 0;
  for (const row of map.learning.data) { const studentId = row?.studentId ? studentIds.get(String(row.studentId)) : null; if (!studentId) unassignedMemory += 1; memoryRows.push(normalizers.memory(row, studentId)); }
  if (unassignedMemory) conflicts.push(conflict("UNASSIGNED_LEGACY_MEMORY", "WARNING", unassignedMemory));
  const attempts = []; const questions = []; const answers = []; const xp = [];
  for (const row of map.quizAttempts.data) {
    const studentId = row?.studentId ? studentIds.get(String(row.studentId)) : null; const attempt = normalizers.attempt(row, studentId); if (!studentId) conflicts.push(conflict("QUIZ_STUDENT_NOT_FOUND", "BLOCKING")); attempts.push(attempt);
  ...
  return { version: "migration-dry-run-v1", mode: "DRY_RUN", capturedAt, sourceRoot: "project-root", snapshot: files, sourceCounts, targetCounts: target, reconciliation: { classroomsToClassrooms: classrooms.length, studentsToStudents: students.length, deferredMemberships: memberships.length, assignedMemory: target.memoryRecords, unassignedMemory, quizAttempts: target.quizAttempts, quizQuestions: target.quizQuestions, quizAnswers: target.quizAnswers, xpEvents: target.xpEvents }, mapping: { school: { logicalId: "legacy-pilot-tenant", deterministicId: schoolId }, classrooms: classrooms.length, students: students.length, attempts: attempts.length }, importOrder: ["migration-tenant metadata", "classrooms", "students", "memberships after account claim", "memory_records", "quiz_attempts", "quiz_attempt_questions", "quiz_attempt_answers", "xp_events"], conflicts: aggregateConflicts(conflicts), blockingConflicts: blocking, warnings, migrationPlanValid: blocking === 0, apply: { supported: false, reason: "APPLY_DISABLED_IN_PHASE_4", safeguards: ["--apply", "--snapshot-hash", "--confirm"] }, transactionPlan: "Future apply should use one transaction for pilot-sized data or bounded domain batches for larger datasets.", database: { used: false, testDatabaseOnly: true }, security: { absolutePaths: false, contentIncluded: false, credentialsIncluded: false } };
}
...
function cli(argv = process.argv.slice(2)) { if (argv.includes("--apply")) throw new Error("APPLY_DISABLED_IN_PHASE_4"); const rootArg = argv.find(value => value.startsWith("--root=")); const root = rootArg ? path.resolve(rootArg.slice(7)) : process.cwd(); const report = buildDryRun(root); const file = writeReport(report); process.stdout.write(JSON.stringify({ ok: true, mode: report.mode, migrationPlanValid: report.migrationPlanValid, blockingConflicts: report.blockingConflicts, warnings: report.warnings, reportFile: path.relative(process.cwd(), file).replace(/\\/g, "/") }) + "\n"); }
