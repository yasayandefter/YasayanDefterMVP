...
function buildDryRun(root = process.cwd(), options = {}) {
  ...
  for (const row of map.learning.data) { const studentId = row?.studentId ? studentIds.get(String(row.studentId)) : mapLegacyDefault ? legacyStudentId : null; if (!studentId) unassignedMemory += 1; memoryRows.push(normalizers.memory(row, studentId)); }
  ...
  for (const row of map.quizAttempts.data) {
    const studentId = row?.studentId ? studentIds.get(String(row.studentId)) : mapLegacyDefault ? legacyStudentId : null; const attempt = normalizers.attempt(row, studentId); if (!studentId) conflicts.push(conflict("QUIZ_STUDENT_NOT_FOUND", "BLOCKING")); attempts.push(attempt);
  ...
  return { version: "migration-dry-run-v1", mode: "DRY_RUN", capturedAt, sourceRoot: "project-root", snapshot: files, sourceCounts, targetCounts: target, reconciliation: { classroomsToClassrooms: classrooms.length, studentsToStudents: students.length, deferredMemberships: memberships.length, assignedMemory: target.memoryRecords, unassignedMemory, quizAttempts: target.quizAttempts, quizQuestions: target.quizQuestions, quizAnswers: target.quizAnswers, xpEvents: target.xpEvents }, ownership: { enabled: mapLegacyDefault, strategy: mapLegacyDefault ? "map_to_legacy_student" : "none", legacyStudentId: mapLegacyDefault ? legacyStudentId : null, mappedMemoryCount: mapLegacyDefault ? unassignedMemory : 0, mappedQuizAttemptCount: mapLegacyDefault ? map.quizAttempts.data.filter(row => !row?.studentId).length : 0, createsUser: false, userId: null, classroomCreated: false }, mapping: { school: { logicalId: "legacy-pilot-tenant", deterministicId: schoolId }, classrooms: classrooms.length, students: students.length, attempts: attempts.length }, importOrder: ["migration-tenant metadata", "classrooms", "students", "memberships after account claim", "memory_records", "quiz_attempts", "quiz_attempt_questions", "quiz_attempt_answers", "xp_events"], conflicts: aggregateConflicts(conflicts), blockingConflicts: blocking, warnings, migrationPlanValid: blocking === 0, apply: { supported: false, reason: "APPLY_DISABLED_IN_PHASE_4", safeguards: ["--apply", "--snapshot-hash", "--confirm", "--confirm-legacy-owner"] }, transactionPlan: "Future apply should use one transaction for pilot-sized data or bounded domain batches for larger datasets.", database: { used: false, testDatabaseOnly: true }, security: { absolutePaths: false, contentIncluded: false, credentialsIncluded: false } };
}
...
function cli(argv = process.argv.slice(2)) { if (argv.includes("--apply")) throw new Error("APPLY_DISABLED_IN_PHASE_4"); const rootArg = argv.find(value => value.startsWith("--root=")); const root = rootArg ? path.resolve(rootArg.slice(7)) : process.cwd(); const report = buildDryRun(root, { mapLegacyDefault: argv.includes("--map-legacy-default") }); const file = writeReport(report); process.stdout.write(JSON.stringify({ ok: true, mode: report.mode, ownershipMapping: report.ownership.enabled, migrationPlanValid: report.migrationPlanValid, blockingConflicts: report.blockingConflicts, warnings: report.warnings, reportFile: path.relative(process.cwd(), file).replace(/\\/g, "/") }) + "\n"); }
