...
const state = {
  ...
  routes: {},
  research: { standard: 0, current: 0, fallback: 0, providerFailures: 0, durationsMs: [] },
...
function reset() { for (const key of Object.keys(state)) { if (key === "startedAt") continue; if (Array.isArray(state[key])) state[key].length = 0; else if (state[key] && typeof state[key] === "object") for (const child of Object.keys(state[key])) delete state[key][child]; } state.startedAt = new Date().toISOString(); state.requests = { total: 0, errors: 0 }; state.research = { standard: 0, current: 0, fallback: 0, providerFailures: 0, durationsMs: [] }; state.quiz = { starts: 0, answers: 0, completions: 0, duplicateAnswers: 0, duplicateCompletions: 0, storageFailures: 0, durationsMs: [] }; state.storage = {}; state.cache = { hit: 0, miss: 0, expired: 0 }; state.errors = {}; }

module.exports = { state, requestStarted, requestCompleted, recordStorage, recordProvider, incrementError, summary, reset, duration };
