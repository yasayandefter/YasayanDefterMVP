...
const state = {
  ...
  storage: {},
  providers: {},
  claims: { count: 0, crossSource: 0, contradictions: 0, quizVerifiedQuestions: 0, eventCrossSource: 0, bilingualMatches: 0, signatureMatches: 0, matchRejected: 0, sameOrgMultiSource: 0 },
...
function recordProvider(provider, durationMs, success, itemCount = 0, cacheHit = false, health = {}) { const key = safeName(provider); state.providers[key] ||= { calls: 0, failures: 0, timeouts: 0, cooldowns: 0, recoveries: 0, cacheHits: 0, itemCount: 0, durationsMs: [] }; const item = state.providers[key]; item.calls += 1; if (!success) item.failures += 1; if (health.timeouts > item.timeouts) item.timeouts = health.timeouts; item.cooldowns = health.status === "cooldown" ? item.cooldowns + 1 : item.cooldowns; item.recoveries = Number(health.recoveries) || item.recoveries; item.healthBucket = health.status || "healthy"; if (cacheHit) item.cacheHits += 1; item.itemCount += Number(itemCount) || 0; duration(item.durationsMs, durationMs); }
function recordClaims(claims = [], contradictions = [], quiz = null) { state.claims.count += claims.length; state.claims.crossSource += claims.filter(claim => claim.independentDomains >= 2).length; state.claims.contradictions += contradictions.length; if (quiz?.verified) state.claims.quizVerifiedQuestions += 1; }
function recordClaimMatching(value = {}) { state.claims.eventCrossSource += Number(value.eventCrossSource) || 0; state.claims.bilingualMatches += Number(value.bilingualMatches) || 0; state.claims.signatureMatches += Number(value.signatureMatches) || 0; state.claims.matchRejected += Number(value.rejected) || 0; state.claims.sameOrgMultiSource += Number(value.sameOrgMultiSource) || 0; }
