...
function answer(attemptId, questionId, answer, skipped = false, studentId = "") {
  ...
  if (!session.ephemeral && !persist()) { session.answers.pop(); return { error: "STORAGE_FAILED" }; }
...
function complete(attemptId, studentId = "") {
  ...
  if (!session.ephemeral && !persist()) { session.completed = false; session.completedAt = null; session.xpAwarded = 0; delete session.summary; return { error: "STORAGE_FAILED" }; }
