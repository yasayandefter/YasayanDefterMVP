...
function createB2Storage(config, dependencies = {}) {
  const api = sdk(dependencies);
  const client = dependencies.client || new api.S3Client({
  ...
  return Object.freeze({
    ...
    async deleteObject({ key }) {
      await client.send(new api.DeleteObjectCommand({ Bucket: config.bucket, Key: key }));
      return { deleted: true };
    }
  });
}

module.exports = { createB2Storage };
