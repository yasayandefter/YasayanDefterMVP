(function () {
  ...
  function renderNote(member, collectionId, host, status) {
    var card = el("article", { class: "collection-member yd-collection-content-card", "data-content-kind": "note", "data-search-text": (member.title || "").toLocaleLowerCase("tr-TR") }), visual = el("div", { class: "yd-content-visual yd-content-visual-note", "aria-hidden": "true" }, "N"), type = el("span", { class: "yd-content-kind" }, "Not"), title = el("strong", {}, member.title), meta = el("span", { class: "yd-content-meta" }, member.contentType || "Not"), remove = el("button", { type: "button", "aria-label": member.title + " kaydını koleksiyondan çıkar" }, "Koleksiyondan çıkar");
    ...
    card.append(visual, type, title, meta, remove); host.append(card);
  }
  ...
  async function openDetail(id) {
    ...
    try {
      ...
      var header = el("header", { class: "yd-collection-detail-head" }), heading = el("div", { class: "yd-collection-heading" }), close = el("button", { type: "button", "aria-label": "Kapat", class: "yd-collection-back" }, "Geri"), headActions = el("div", { class: "yd-collection-head-actions" }), edit = el("button", { type: "button" }, "Bilgileri Düzenle"), destroy = el("button", { type: "button", class: "is-danger" }, "Koleksiyonu Sil");
      heading.append(el("p", { class: "yd-collections-eyebrow" }, "KOLEKSİYON"), el("h2", { id: "collectionDetailDialogTitle", tabindex: "-1" }, data.collection.name), el("p", {}, data.collection.description || "Notlarını ve medyanı tek çalışma alanında düzenle.")); headActions.append(edit, destroy); header.append(close, heading, headActions);
      var toolbar = el("div", { class: "yd-collection-content-toolbar" }), addMenu = el("details", { class: "yd-add-content" }), addSummary = el("summary", {}, "İçerik Ekle"), addPanel = el("div", { class: "yd-add-content-menu" }), addSelect = el("select", { "aria-label": "Koleksiyona eklenecek kayıt" }), addNote = el("button", { type: "button" }, "Seçili Notu Ekle"), upload = el("button", { type: "button" }, "Medya Yükle"), search = el("input", { type: "search", placeholder: "İçerikte ara", "aria-label": "Koleksiyon içeriğinde ara" }), filters = el("div", { class: "yd-content-filters", role: "group", "aria-label": "İçerik türü" }), selectedKind = "all";
      [["all", "Tümü"], ["note", "Notlar"], ["document", "Belgeler"], ["image", "Görseller"], ["audio", "Ses"], ["video", "Video"]].forEach(function (entry) { var button = el("button", { type: "button", "data-filter-kind": entry[0], "aria-pressed": entry[0] === "all" ? "true" : "false" }, entry[1]); button.onclick = function () { selectedKind = entry[0]; filters.querySelectorAll("button").forEach(function (item) { item.setAttribute("aria-pressed", String(item === button)); }); applyContentView(); }; filters.append(button); });
      var memberIds = new Set(data.members.map(function (member) { return member.id; })); all.memories.filter(function (memory) { return !memberIds.has(memory.id); }).forEach(function (memory) { addSelect.append(el("option", { value: memory.id }, memory.title)); }); addPanel.append(addSelect, addNote, upload); addMenu.append(addSummary, addPanel); toolbar.append(addMenu, search, filters);
      var body = el("div", { class: "yd-collection-detail-body" }), content = el("main", { class: "yd-collection-content", "aria-label": "Koleksiyon içeriği" }), grid = el("div", { class: "yd-collection-content-grid", "data-content-grid": "unified" }), empty = el("p", { class: "yd-media-empty", hidden: "" }, "Bu görünümde içerik yok."), previewHost = el("aside", { class: "yd-media-preview", "aria-label": "İçerik önizleme", hidden: "" });
      content.append(grid, empty); body.append(content, previewHost);
      var status = el("p", { class: "collection-status yd-collection-detail-status", role: "status", "aria-live": "polite" }); view.form.append(header, toolbar, body, status);
      data.members.forEach(function (member) { renderNote(member, id, grid, status); });
      function applyContentView() { var query = search.value.trim().toLocaleLowerCase("tr-TR"), visible = 0; grid.querySelectorAll(".yd-collection-content-card,.yd-media-card").forEach(function (card) { var kindMatch = selectedKind === "all" || card.dataset.contentKind === selectedKind, queryMatch = !query || (card.dataset.searchText || card.textContent.toLocaleLowerCase("tr-TR")).includes(query); card.hidden = !(kindMatch && queryMatch); if (!card.hidden) visible += 1; }); empty.hidden = visible !== 0; status.textContent = visible + " içerik gösteriliyor."; }
      var mediaMount = await window.YDCollectionMedia?.mount({ collectionId: id, grid: grid, previewHost: previewHost, status: status, onRendered: applyContentView }); applyContentView();
      ...
      search.oninput = applyContentView;
      ...
      view.d.addEventListener("close", function () { mediaMount?.destroy(); }, { once: true });
  ...
  function trap(event) { if (event.key !== "Tab") return; var nodes = [...event.currentTarget.querySelectorAll("button,input,textarea,select,summary,a[href]")].filter(function (node) { var closed = node.closest("details:not([open])"), hiddenByDisclosure = closed && node !== closed.querySelector(":scope > summary"); return !node.disabled && !node.hidden && !hiddenByDisclosure && node.getClientRects().length > 0; }), first = nodes[0], last = nodes[nodes.length - 1]; if (!first) return; if (event.shiftKey && document.activeElement === first) { event.preventDefault(); last.focus(); } else if (!event.shiftKey && document.activeElement === last) { event.preventDefault(); first.focus(); } }
