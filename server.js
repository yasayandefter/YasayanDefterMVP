...
app.get("/api/intelligence/home",async(req,res,next)=>{if(req.repositories?.mode!=="postgres")return next();const owner=requirePersonalIntelligenceContext(req,res);if(owner===null)return;try{const started=Date.now(),[data,feedback,preferences]=await Promise.all([req.repositories.homeIntelligence.candidates(owner),req.repositories.intelligenceFeedback.listRecent(owner),intelligencePreferences(req)]),home=homeIntelligence.build(data,preferences,feedback);return res.json({ok:true,home,meta:{durationMs:Date.now()-started},requestId:req.requestId});}catch(_){return res.status(503).json({ok:false,error:{code:"STORAGE_UNAVAILABLE",message:"Ana sayfa önerileri şu anda oluşturulamadı."},requestId:req.requestId});}});
app.get("/api/intelligence/context",async(req,res,next)=>{if(req.repositories?.mode!=="postgres")return next();const owner=requirePersonalIntelligenceContext(req,res);if(owner===null)return;const id=cleanText(req.query?.recordId||"",100);if(!/^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(id))return res.status(400).json({ok:false,error:{code:"INVALID_CONTEXT_REQUEST",message:"Kayıt kimliği geçersiz."},requestId:req.requestId});try{const target=await req.repositories.memory.findOwnedById(id,owner);if(!target.exists)return res.status(404).json({ok:false,error:{code:"NOT_FOUND",message:"Defter kaydı bulunamadı."},requestId:req.requestId});if(!target.memory)return res.status(403).json({ok:false,error:{code:"FORBIDDEN",message:"Bu kayıt için bağlantı görüntüleme yetkiniz yok."},requestId:req.requestId});const [candidates,feedback,collectionMemberIds,preferences]=await Promise.all([req.repositories.memory.findIntelligenceCandidates(owner,200),req.repositories.intelligenceFeedback.listRecent(owner),req.repositories.smartCollections.relatedMemberIds(id,owner),intelligencePreferences(req)]),context=contextIntelligence.build(target.memory,candidates,preferences,{feedback,collectionMemberIds});return res.json({ok:true,context,requestId:req.requestId});}catch(_){return res.status(503).json({ok:false,error:{code:"STORAGE_UNAVAILABLE",message:"Bağlantılar şu anda oluşturulamadı."},requestId:req.requestId});}});
...
app.post("/api/intelligence/feedback",async(req,res,next)=>{if(req.repositories?.mode!=="postgres")return next();const owner=requirePersonalIntelligenceContext(req,res,req.body||{});if(owner===null)return;try{const value=smartCollection.feedback(req.body);for(const id of [value.contextRecordId,value.targetRecordId].filter(Boolean)){const record=await req.repositories.memory.findOwnedById(id,owner);if(!record.exists)return res.status(404).json({ok:false,error:{code:"NOT_FOUND",message:"Kayıt bulunamadı."},requestId:req.requestId});if(!record.memory)return res.status(403).json({ok:false,error:{code:"FORBIDDEN",message:"Bu öneri için geri bildirim yetkiniz yok."},requestId:req.requestId});}return res.json({ok:true,feedback:await req.repositories.intelligenceFeedback.upsert(owner,value),requestId:req.requestId});}catch(error){const invalid=error?.code==="INVALID_SMART_COLLECTION";return res.status(invalid?400:503).json({ok:false,error:{code:invalid?error.code:"STORAGE_UNAVAILABLE",message:invalid?"Geri bildirim geçersiz.":"Geri bildirim kaydedilemedi."},requestId:req.requestId});}});

app.get("/api/collections",async(req,res,next)=>{if(req.repositories?.mode!=="postgres")return next();const owner=requirePersonalIntelligenceContext(req,res);if(owner===null)return;try{return res.json({ok:true,collections:await req.repositories.smartCollections.list(owner,smartCollection.list(req.query||{})),requestId:req.requestId});}catch(error){const invalid=error?.code==="INVALID_SMART_COLLECTION";return res.status(invalid?400:503).json({ok:false,error:{code:invalid?error.code:"STORAGE_UNAVAILABLE",message:invalid?"Koleksiyon filtresi geçersiz.":"Koleksiyonlar alınamadı."},requestId:req.requestId});}});
app.post("/api/collections",async(req,res,next)=>{if(req.repositories?.mode!=="postgres")return next();const owner=requirePersonalIntelligenceContext(req,res,req.body||{});if(owner===null)return;try{const value=smartCollection.collection(req.body),existing=await req.repositories.smartCollections.list(owner,{limit:50,sort:"updated"}),result=await req.repositories.smartCollections.create(owner,value);if(!result.collection)return res.status(403).json({ok:false,error:{code:"FORBIDDEN",message:"Koleksiyon yalnız kendi Defterim kayıtlarınla oluşturulabilir."},requestId:req.requestId});const duplicateWarning=existing.some(x=>contextIntelligence.normalize(x.name)===contextIntelligence.normalize(value.name));return res.status(201).json({ok:true,collection:result.collection,duplicateWarning,requestId:req.requestId});}catch(error){const invalid=error?.code==="INVALID_SMART_COLLECTION";return res.status(invalid?400:503).json({ok:false,error:{code:invalid?error.code:"STORAGE_UNAVAILABLE",message:invalid?"Koleksiyon alanları geçersiz.":"Koleksiyon oluşturulamadı."},requestId:req.requestId});}});
app.get("/api/collections/:id",async(req,res,next)=>{if(req.repositories?.mode!=="postgres")return next();const owner=requirePersonalIntelligenceContext(req,res);if(owner===null)return;let id;try{id=smartCollection.uuid(req.params.id);}catch(_){return res.status(404).json({ok:false,error:{code:"NOT_FOUND",message:"Koleksiyon bulunamadı."},requestId:req.requestId});}try{const result=await req.repositories.smartCollections.findOwned(id,owner);if(!result.exists)return res.status(404).json({ok:false,error:{code:"NOT_FOUND",message:"Koleksiyon bulunamadı."},requestId:req.requestId});if(!result.collection)return res.status(403).json({ok:false,error:{code:"FORBIDDEN",message:"Bu koleksiyonu görüntüleme yetkiniz yok."},requestId:req.requestId});return res.json({ok:true,...result,requestId:req.requestId});}catch(_){return res.status(503).json({ok:false,error:{code:"STORAGE_UNAVAILABLE",message:"Koleksiyon alınamadı."},requestId:req.requestId});}});
app.patch("/api/collections/:id",async(req,res,next)=>{if(req.repositories?.mode!=="postgres")return next();const owner=requirePersonalIntelligenceContext(req,res,req.body||{});if(owner===null)return;try{const id=smartCollection.uuid(req.params.id),value=smartCollection.collection(req.body,true);if(!Object.keys(value).length)throw smartCollection.invalid();const result=await req.repositories.smartCollections.update(id,owner,value);if(!result.exists)return res.status(404).json({ok:false,error:{code:"NOT_FOUND",message:"Koleksiyon bulunamadı."},requestId:req.requestId});if(!result.collection)return res.status(403).json({ok:false,error:{code:"FORBIDDEN",message:"Bu koleksiyonu düzenleme yetkiniz yok."},requestId:req.requestId});return res.json({ok:true,collection:result.collection,requestId:req.requestId});}catch(error){const invalid=error?.code==="INVALID_SMART_COLLECTION";return res.status(invalid?400:503).json({ok:false,error:{code:invalid?error.code:"STORAGE_UNAVAILABLE",message:invalid?"Koleksiyon alanları geçersiz.":"Koleksiyon güncellenemedi."},requestId:req.requestId});}});
app.post("/api/collections/:id/items",async(req,res,next)=>{if(req.repositories?.mode!=="postgres")return next();const owner=requirePersonalIntelligenceContext(req,res,req.body||{});if(owner===null)return;try{const id=smartCollection.uuid(req.params.id),recordIds=smartCollection.ids(req.body?.recordIds);if(Object.keys(req.body||{}).some(k=>k!=="recordIds"))throw smartCollection.invalid();const result=await req.repositories.smartCollections.addItems(id,owner,recordIds);if(!result.collection_exists)return res.status(403).json({ok:false,error:{code:"FORBIDDEN",message:"Koleksiyon erişimi reddedildi."},requestId:req.requestId});if(Number(result.owned_count)!==recordIds.length)return res.status(403).json({ok:false,error:{code:"FORBIDDEN",message:"Yalnız kendi kayıtlarını ekleyebilirsin."},requestId:req.requestId});if(!result.capacity_ok&&Number(result.candidate_count)>0)return res.status(409).json({ok:false,error:{code:"COLLECTION_CAPACITY_EXCEEDED",message:"Koleksiyon en fazla 100 öğe içerebilir."},requestId:req.requestId});return res.json({ok:true,added:Number(result.added_count),requestId:req.requestId});}catch(error){const invalid=error?.code==="INVALID_SMART_COLLECTION";return res.status(invalid?400:503).json({ok:false,error:{code:invalid?error.code:"STORAGE_UNAVAILABLE",message:invalid?"Üyelik isteği geçersiz.":"Kayıt eklenemedi."},requestId:req.requestId});}});
app.get("/api/collections/:id/media",async(req,res,next)=>{if(req.repositories?.mode!=="postgres")return next();const owner=requirePersonalIntelligenceContext(req,res);if(owner===null)return;try{const result=await req.repositories.smartCollections.listMedia(smartCollection.uuid(req.params.id),owner);if(!result.exists)return res.status(404).json({ok:false,error:{code:"NOT_FOUND",message:"Koleksiyon bulunamadı."},requestId:req.requestId});if(!result.owned)return res.status(403).json({ok:false,error:{code:"FORBIDDEN",message:"Koleksiyon erişimi reddedildi."},requestId:req.requestId});return res.json({ok:true,media:result.media,requestId:req.requestId});}catch(error){const invalid=error?.code==="INVALID_SMART_COLLECTION";return res.status(invalid?404:503).json({ok:false,error:{code:invalid?"NOT_FOUND":"STORAGE_UNAVAILABLE",message:invalid?"Koleksiyon bulunamadı.":"Koleksiyon medyası alınamadı."},requestId:req.requestId});}});
app.post("/api/collections/:id/media",async(req,res,next)=>{if(req.repositories?.mode!=="postgres")return next();const owner=requirePersonalIntelligenceContext(req,res,req.body||{});if(owner===null)return;try{const id=smartCollection.uuid(req.params.id),mediaAssetIds=smartCollection.ids(req.body?.mediaAssetIds);if(Object.keys(req.body||{}).some(k=>k!=="mediaAssetIds"))throw smartCollection.invalid();const result=await req.repositories.smartCollections.addMedia(id,owner,req.auth.userId,mediaAssetIds);if(!result.collection_exists)return res.status(403).json({ok:false,error:{code:"FORBIDDEN",message:"Koleksiyon erişimi reddedildi."},requestId:req.requestId});if(Number(result.owned_count)!==mediaAssetIds.length)return res.status(403).json({ok:false,error:{code:"FORBIDDEN",message:"Yalnız kendi hazır medyanı ekleyebilirsin."},requestId:req.requestId});if(!result.capacity_ok&&Number(result.candidate_count)>0)return res.status(409).json({ok:false,error:{code:"COLLECTION_CAPACITY_EXCEEDED",message:"Koleksiyon en fazla 100 öğe içerebilir."},requestId:req.requestId});return res.json({ok:true,added:Number(result.added_count),requestId:req.requestId});}catch(error){const invalid=error?.code==="INVALID_SMART_COLLECTION";return res.status(invalid?400:503).json({ok:false,error:{code:invalid?error.code:"STORAGE_UNAVAILABLE",message:invalid?"Medya üyeliği isteği geçersiz.":"Medya eklenemedi."},requestId:req.requestId});}});
app.delete("/api/collections/:id/media/:mediaId",async(req,res,next)=>{if(req.repositories?.mode!=="postgres")return next();const owner=requirePersonalIntelligenceContext(req,res);if(owner===null)return;try{const result=await req.repositories.smartCollections.removeMedia(smartCollection.uuid(req.params.id),smartCollection.uuid(req.params.mediaId),owner,req.auth.userId);if(!result.exists)return res.status(404).json({ok:false,error:{code:"NOT_FOUND",message:"Koleksiyon bulunamadı."},requestId:req.requestId});if(!result.owned||!result.asset_owned)return res.status(403).json({ok:false,error:{code:"FORBIDDEN",message:"Koleksiyon veya medya erişimi reddedildi."},requestId:req.requestId});if(!result.removed)return res.status(404).json({ok:false,error:{code:"NOT_FOUND",message:"Medya koleksiyonda bulunamadı."},requestId:req.requestId});return res.json({ok:true,removed:true,requestId:req.requestId});}catch(error){const invalid=error?.code==="INVALID_SMART_COLLECTION";return res.status(invalid?404:503).json({ok:false,error:{code:invalid?"NOT_FOUND":"STORAGE_UNAVAILABLE",message:"Medya koleksiyondan çıkarılamadı."},requestId:req.requestId});}});
app.delete("/api/collections/:id/items/:recordId",async(req,res,next)=>{if(req.repositories?.mode!=="postgres")return next();const owner=requirePersonalIntelligenceContext(req,res);if(owner===null)return;try{const result=await req.repositories.smartCollections.removeItem(smartCollection.uuid(req.params.id),smartCollection.uuid(req.params.recordId),owner);if(!result.exists)return res.status(404).json({ok:false,error:{code:"NOT_FOUND",message:"Koleksiyon bulunamadı."},requestId:req.requestId});if(!result.owned)return res.status(403).json({ok:false,error:{code:"FORBIDDEN",message:"Koleksiyon erişimi reddedildi."},requestId:req.requestId});if(!result.removed)return res.status(404).json({ok:false,error:{code:"NOT_FOUND",message:"Kayıt koleksiyonda bulunamadı."},requestId:req.requestId});return res.json({ok:true,removed:true,requestId:req.requestId});}catch(error){const invalid=error?.code==="INVALID_SMART_COLLECTION";return res.status(invalid?404:503).json({ok:false,error:{code:invalid?"NOT_FOUND":"STORAGE_UNAVAILABLE",message:"Kayıt çıkarılamadı."},requestId:req.requestId});}});
app.delete("/api/collections/:id",async(req,res,next)=>{if(req.repositories?.mode!=="postgres")return next();const owner=requirePersonalIntelligenceContext(req,res);if(owner===null)return;try{const result=await req.repositories.smartCollections.deleteOwned(smartCollection.uuid(req.params.id),owner);if(!result.exists)return res.status(404).json({ok:false,error:{code:"NOT_FOUND",message:"Koleksiyon bulunamadı."},requestId:req.requestId});if(!result.deleted)return res.status(403).json({ok:false,error:{code:"FORBIDDEN",message:"Koleksiyon erişimi reddedildi."},requestId:req.requestId});return res.json({ok:true,deleted:true,requestId:req.requestId});}catch(_){return res.status(503).json({ok:false,error:{code:"STORAGE_UNAVAILABLE",message:"Koleksiyon silinemedi."},requestId:req.requestId});}});
...
app.get("/api/memory/list", async (req, res, next) => {
  ...
  try { const options=noteOrganization.list(req.query||{});const memories=await req.repositories.memory.findByStudent(studentId,options);return res.json({ok:true,memories,page:{offset:options.offset,limit:options.limit,hasMore:Boolean(memories.hasMore),nextOffset:memories.hasMore?options.offset+options.limit:null},requestId:req.requestId}); }
  catch (error) { if(error?.code==="INVALID_NOTE_ORGANIZATION")return res.status(400).json({ok:false,error:{code:error.code,message:"Defter filtresi geçersiz."},requestId:req.requestId});return res.status(503).json({ok:false,error:{code:"STORAGE_UNAVAILABLE",message:"Hafıza kayıtları şu anda alınamadı."},requestId:req.requestId}); }
});
...
app.patch("/api/memory/:id",async(req,res,next)=>{
  ...
  try{const organization=noteOrganization.patch(req.body||{}),hasSmart=["title","customTitle","content","workspaceArea","contentType","tags","metadata"].some(key=>Object.hasOwn(req.body||{},key));let result;if(hasSmart){const value=smartNote.normalize({title:req.body?.customTitle||req.body?.title||"Defter kaydı",content:req.body?.content||"Korunan araştırma içeriği",workspaceArea:req.body?.workspaceArea,contentType:req.body?.contentType,tags:req.body?.tags,metadata:req.body?.metadata});result=await req.repositories.memory.updateOwned(id,owner,value);}if(Object.keys(organization).length)result=await req.repositories.memory.organizeOwned(id,owner,organization);if(!result?.exists)return res.status(404).json({ok:false,error:{code:"NOT_FOUND",message:"Defter kaydı bulunamadı."},requestId:req.requestId});if(!result.memory)return res.status(403).json({ok:false,error:{code:"FORBIDDEN",message:"Bu kaydı düzenleme yetkiniz yok."},requestId:req.requestId});return res.json({ok:true,memory:result.memory,requestId:req.requestId});}
  catch(error){const invalid=["INVALID_SMART_NOTE","INVALID_NOTE_ORGANIZATION"].includes(error?.code);return res.status(invalid?400:503).json({ok:false,error:{code:invalid?error.code:"STORAGE_UNAVAILABLE",message:invalid?"Not alanları geçersiz.":"Not şu anda güncellenemedi."},requestId:req.requestId});}
});

app.get("/api/memory/filter-presets",async(req,res,next)=>{if(req.repositories?.mode!=="postgres")return next();if(requireStudentContext(req,res)===null)return;try{return res.json({ok:true,presets:await req.repositories.noteFilterPresets.list(req.auth.userId),requestId:req.requestId});}catch(_){return res.status(503).json({ok:false,error:{code:"STORAGE_UNAVAILABLE",message:"Kaydedilmiş görünümler alınamadı."},requestId:req.requestId});}});
app.post("/api/memory/filter-presets",async(req,res,next)=>{if(req.repositories?.mode!=="postgres")return next();if(requireStudentContext(req,res,req.body||{})===null)return;try{const value=noteOrganization.preset(req.body),preset=await req.repositories.noteFilterPresets.create(req.auth.userId,value);if(!preset)return res.status(409).json({ok:false,error:{code:"PRESET_LIMIT",message:"En fazla 8 görünüm kaydedebilirsin."},requestId:req.requestId});return res.status(201).json({ok:true,preset,requestId:req.requestId});}catch(error){const invalid=error?.code==="INVALID_NOTE_ORGANIZATION";return res.status(invalid?400:503).json({ok:false,error:{code:invalid?error.code:"STORAGE_UNAVAILABLE",message:invalid?"Görünüm alanları geçersiz.":"Görünüm kaydedilemedi."},requestId:req.requestId});}});
app.delete("/api/memory/filter-presets/:id",async(req,res,next)=>{if(req.repositories?.mode!=="postgres")return next();if(requireStudentContext(req,res)===null)return;const id=cleanText(req.params.id||"",100);if(!/^[0-9a-f-]{36}$/i.test(id))return res.status(404).json({ok:false,error:{code:"NOT_FOUND",message:"Görünüm bulunamadı."},requestId:req.requestId});try{const result=await req.repositories.noteFilterPresets.deleteOwned(id,req.auth.userId);if(!result.exists)return res.status(404).json({ok:false,error:{code:"NOT_FOUND",message:"Görünüm bulunamadı."},requestId:req.requestId});if(!result.deleted)return res.status(403).json({ok:false,error:{code:"FORBIDDEN",message:"Bu görünümü silme yetkiniz yok."},requestId:req.requestId});return res.json({ok:true,deleted:true,id,requestId:req.requestId});}catch(_){return res.status(503).json({ok:false,error:{code:"STORAGE_UNAVAILABLE",message:"Görünüm silinemedi."},requestId:req.requestId});}});
