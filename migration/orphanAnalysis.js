...
function analyzeOrphans(root, attempts, currentStudentIds) {
  ...
  const recoverable = []; const consistent = []; const unresolved = []; const conflicting = [];
  for (const [id, count] of counts) { const evidence = backupEvidence.get(id); if (evidence?.size > 1) conflicting.push(id); else if (evidence?.size === 1) recoverable.push(id); else if (count > 1) consistent.push(id); else unresolved.push(id); }
  return { missingStudentIdCount: counts.size, affectedQuizAttemptCount: [...counts.values()].reduce((sum, value) => sum + value, 0), recoverableFromBackup: recoverable.length, consistentOrphans: consistent.length, unresolvedOrphans: unresolved.length, conflictingOrphans: conflicting.length, groups: { recoverable: recoverable.map(masked), consistent: consistent.map(masked), unresolved: unresolved.map(masked), conflicting: conflicting.map(masked) }, counts: Object.fromEntries([...counts.entries()].map(([id, count]) => [masked(id), count])), timestampRanges: Object.fromEntries([...timestamps.entries()].map(([id, range]) => [masked(id), range])), mappings: new Map([...recoverable, ...consistent].map(id => [id, id])) };
}
